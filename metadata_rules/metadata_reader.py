...
class MetadataReader:
    ...
    # TODO: Forhåpenligvis kan vi fjerne set_cookie() hvis de får åpnet Concept LDS API
    def set_cookie(self):
        self.__lds_cookie = input("Cookie:")


    # Leser metadata fra en Jupyter %document-json-fil 
    def read_dapla_jupyter_metadata(self, jupyter_meta_filename:str):
        # TODO: Lese metadata fra Jupyter i Dapla notebook miljø.
        path_jupyter_doc_json = self.__project_root_path.joinpath("tests").joinpath("resources").joinpath("example_metadata")

        with open(path_jupyter_doc_json.joinpath(jupyter_meta_filename)) as json_file: 
    ...
    def get_instance_variable_represented_variable(self, name):
        return self.get_instance_variable(name)["representedVariable"]["selected-enum"]

    def get_instance_variable_population(self, name):
        return self.get_instance_variable(name)["population"]["selected-enum"]

    def get_instance_variable_sentinel_value_domain(self, name):
        return self.get_instance_variable(name)["sentinelValueDomain"]["selected-enum"]


    def get_represented_variable(self):
        # TODO: retun instance variable object 
        pass


    def get_concept_metadata(self, object_type:str, object_id:str=None):
        concept_lds_url = "https://dapla-workbench.staging-bip-app.ssb.no/be/concept-lds/ns/"
        concept_object_url = concept_lds_url + object_type
        if object_id:
            concept_object_url = concept_object_url + "/" + object_id
        #concept_lds_result = requests.get('https://dapla-workbench.staging-bip-app.ssb.no/be/concept-lds/ns/RepresentedVariable', headers={"Cookie": self.__lds_cookie} )
        return requests.get(concept_object_url, headers={"Cookie": self.__lds_cookie} )



    def read_data_concept_metadata(self):
        # TODO: Lese metadata fra Concept LDS API
    ...
    def read_all_metadata(self):
        pass
        #self.read_dapla_jupyter_metadata("personinntekt_eksempel.json")
        #self.read_data_concept_metadata()


### Eksempel på bruk:
mr = MetadataReader()
mr.read_dapla_jupyter_metadata("personinntekt_eksempel.json")
#print(json.dumps(mr.jupyter_meta, sort_keys=False, indent=4))
print(mr.get_dataset_name())
print(mr.get_dataset_description())
print(mr.get_unit_type())
#print(mr.get_instance_variables_list())
print(mr.get_instance_variable_component_type("zzzfnr"))

#print(mr.get_concept_metadata("Population").text)
#mr.read_all_metadata()
